...
| Doc | Mục đích | Trạng thái | Cập nhật |
|-----|----------|-----------|----------|
| [`specs/voicekey-a11y-bind-recovery.html`](specs/voicekey-a11y-bind-recovery.html) | **Phím-thoại: cứu kết nối trợ năng khi AMS kẹt** — nhận ra ca `Binding services` kẹt (toggle setting VÔ ÍCH, [ĐO] trên xe 2026-09-11) và escalate sang khởi động lại tiến trình + tự mở lại; nút "Kiểm tra / Sửa ngay" đi được tới bước cuối đó (có xin phép), boot nền tự làm ở bước cuối. Gỡ hằng số component còn ghi package cũ | **Current** | 2026-09-11 |
| [`specs/permission-health-audit.html`](specs/permission-health-audit.html) | **Kiểm tra quyền tự động (boot + mở app)** — đọc TRƯỚC trạng thái thật của 9 quyền/cấu hình ngoài app (listener · a11y · overlay self · `byd_float_app_list` self+VietMap · appop VietMap · trợ lý hệ thống · doze self+VietMap), đủ ⇒ im lặng không cấp lại, thiếu ⇒ tự vá + đọc lại, vá không được ⇒ popup xin owner (notification ở nhánh boot, chỉ khi kết luận đổi). Bỏ cờ một-lần `vm_float_whitelist_applied` (gốc bug bóng VietMap vẫn dính "IVI không hỗ trợ" ở v1.38) | **Draft / chờ duyệt · review Pass 1 APPROVED** | 2026-09-10 |
...
| Doc | Mục đích | Trạng thái | Cập nhật |
|-----|----------|-----------|----------|
| [`diagnostics/voicekey-a11y-bind-stuck-2026-09-11.md`](diagnostics/voicekey-a11y-bind-stuck-2026-09-11.md) | **Phím-thoại mất kết nối: AMS kẹt bind** — bằng chứng ĐÃ đo (hình dạng trạng thái kẹt · chuỗi loại trừ · dòng thời gian pid bác giả thuyết OTA) vs 5 câu CHƯA đo (Q1–Q5), checklist đo sẵn-để-dán cho lần lên xe, và 5 hướng xử lý A–E đã cân nhưng CHƯA chọn. **Đừng vá thêm khi chưa có số.** | Current | 2026-09-11 |
