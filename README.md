...
**Current version: 1.40 (versionCode 41) — "Cluster Nav 2.0" (`com.byd.clusternav2`), a standalone app independent of the legacy `com.byd.clusternav` (its own package + its own signing key, installs side by side).** `byd-cluster-2` re-baselines the 1.30 ClusterNav codebase (Waze/VietMap signals revived — see `docs/specs/waze-vietmap-signal-revival.html`) as a fresh **1.0** for a new iteration; the app OTA self-updates from **this** repo's `apk/ClusterNav-<ver>-release.apk` on `main`. The per-version notes below are kept as lineage history.

`1.40` fixes the on-car finding that the physical-button voice key lost its accessibility connection and could not self-heal. **(VI)** `1.40`: sửa lỗi phím-thoại mất kết nối trợ năng mà app tự chữa không được.

- **The stuck bind is now recognised and escalated.** Measured on the car: the accessibility service was listed under `Enabled services`, missing from `Bound services`, and parked in `Binding services` with two dead connection records — the app itself was healthy. The existing self-heal (toggling the service out of and back into the enabled list) ran and failed, and removing the component from the setting entirely left it in `Binding services`, so the stuck state lives in the car's `system_server`, not in the setting. Only killing the app's process released it. ClusterNav now detects that exact state, skips the toggle that cannot work (each toggle drops both the voice key and the distance booster), and escalates to a process restart that relaunches itself. It waits and re-reads first, so a merely slow bind never triggers a restart, and the restart is capped at once per process plus a 10-minute cooldown.
- **"Check / Fix now" reaches that last step.** The button, and opening the app while the voice key is disconnected, now ask for permission and then perform the restart; at engine start it happens by itself (no dialog when nobody is looking) as the last boot step, so nothing else in the boot chain is cut short.

`1.39` adds an automatic permission health check at boot and at app open, and fixes the on-car v1.38 finding that the VietMap cluster bubble was still blocked by the IVI. **(VI)** `1.39`: tự kiểm tra quyền khi nổ máy + khi mở app (đọc trước, tự vá, popup khi cần) + sửa lỗi trên xe "bóng VietMap vẫn bị IVI không hỗ trợ".

- **Permission health check (read first, fix only what is missing).** The app now audits every out-of-app grant its *enabled* features depend on — notification access, accessibility, its own overlay permission, the car's `byd_float_app_list` membership for itself and for VietMap, VietMap's overlay appop, the Google/Gemini system-assistant settings, and doze exemptions. It reads the real state first: when everything is already granted it issues **no write command at all** (and does not even open the local ADB session when nothing needs a shell read). Only what is missing gets fixed, and every write is followed by a read-back, because a rejected `settings put` prints an error instead of throwing. What the app cannot grant itself (the one-time "Allow USB debugging" tap) is surfaced: a dialog when you open the app, or a boot notification — and the boot notification only fires when the conclusion *changes*, so it never nags on every ignition.
- **VietMap bubble: the v1.35 whitelist no longer gets locked out.** On v1.38 the whitelist recipe was gated by a one-shot preference flag, so re-installing the modded VietMap (a different signing key means uninstall+install, which wipes its `SYSTEM_ALERT_WINDOW` appop) left the bubble blocked forever — the flag said "already applied". The flag is gone; state is read from the device every time. And because the mod only builds its bubble at process start, a freshly repaired grant now force-stops VietMap and relaunches it (bypassing the autostart cooldown) so the bubble actually appears.
